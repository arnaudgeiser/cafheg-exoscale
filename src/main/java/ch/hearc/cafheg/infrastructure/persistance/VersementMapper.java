...
public class VersementMapper extends Mapper {
  ...
  private static final String QUERY_FIND_ALL_ALLOCATIONS_NAISSANCE = "SELECT V.DATE_VERSEMENT,AN.MONTANT FROM VERSEMENTS V JOIN ALLOCATIONS_NAISSANCE AN ON V.NUMERO=AN.FK_VERSEMENTS";
  private static final String QUERY_FIND_ALL_VERSEMENTS = "SELECT V.DATE_VERSEMENT,A.MONTANT FROM VERSEMENTS V JOIN VERSEMENTS_ALLOCATIONS VA ON V.NUMERO=VA.FK_VERSEMENTS JOIN ALLOCATIONS_ENFANTS AE ON AE.NUMERO=VA.FK_ALLOCATIONS_ENFANTS JOIN ALLOCATIONS A ON A.NUMERO=AE.FK_ALLOCATIONS";
  ...
  public List<VersementAllocationNaissance> findAllVersementAllocationNaissance() {
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(
        QUERY_FIND_ALL_ALLOCATIONS_NAISSANCE)) {
  ...
  public List<VersementParentEnfant> findVersementParentEnfant() {
    ...
    Connection connection = activeJDBCConnection();
    try (PreparedStatement preparedStatement = connection.prepareStatement(
        QUERY_FIND_ALL_VERSEMENTS_PARENTS_ENFANTS)) {
      ResultSet resultSet = preparedStatement.executeQuery();
  ...
  public List<VersementParentParMois> findVersementParentEnfantParMois() {
    ...
    Connection connection = activeJDBCConnection();
    try (PreparedStatement preparedStatement = connection.prepareStatement(
        QUERY_FIND_ALL_VERSEMENTS_PARENTS_ENFANTS_PAR_MOIS)) {
      ResultSet resultSet = preparedStatement.executeQuery();
